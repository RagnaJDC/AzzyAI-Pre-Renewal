...
namespace AzzyAIConfig
{
    static class H_Config
    {
        ...
        static void LoadBasicOptions(string file)
        {
            ...
            if (Regex.IsMatch(file, "PickUpBraggi\\s*=\\s*-?\\d+", RegexOptions.Multiline))
            {
                PickUpBraggi = Convert.ToInt32(Regex.Match(file, "PickUpBraggi\\s*=\\s*-?\\d+", RegexOptions.Multiline).Value.Split('=')[1].Trim());
            }
            if (Regex.IsMatch(file, "PickUpDancer\\s*=\\s*-?\\d+", RegexOptions.Multiline))
            {
                PickUpDancer = Convert.ToInt32(Regex.Match(file, "PickUpDancer\\s*=\\s*-?\\d+", RegexOptions.Multiline).Value.Split('=')[1].Trim());
            }
        ...
        static void LoadAutoSkillOptions(string file)
        {
            ...
            if (Regex.IsMatch(file, "UseSkillOnFollow\\s*=\\s*-?\\d+", RegexOptions.Multiline))
            {
                UseSkillOnFollow = Convert.ToInt32(Regex.Match(file, "UseSkillOnFollow\\s*=\\s*-?\\d+", RegexOptions.Multiline).Value.Split('=')[1].Trim());
            }
            ...
            // Persistence for EiraSilentBreezeLevel removed (handled by AI), not read from GUI file.
            ...
            // Persistence for BayeriStahlHornLevel removed (handled by AI), not read from GUI file.
            ...
            // Persistence for BayeriHailegeStarLevel removed (handled by AI), not read from GUI file.
            ...
            // Persistence for SeraParalyzeLevel removed (handled by AI), not read from GUI file.
            ...
            // Persistence for SeraPoisonMistLevel removed (handled by AI), not read from GUI file.
            ...
            // Persistence for EleanorSonicClawLevel removed (handled by AI), not read from GUI file.
            // Persistence for EleanorSilverveinLevel removed (handled by AI), not read from GUI file.
            // Persistence for EleanorMidnightLevel removed (handled by AI), not read from GUI file.
            ...
            // Persistence for DieterLavaSlideLevel removed (handled by AI), not read from GUI file.
        ...
        static void LoadAutobuffOptions(string file)
        {
            ...
            // Persistence for BayeriSteinWandLevel removed (handled by AI), not read from GUI file.
        ...
        static string SaveBasicOptions(string file)
        {
            ...
            if (Regex.IsMatch(file, "PickUpBraggi\\s*=\\s*(\\d+|-\\d+)", RegexOptions.Multiline))
            {
                file = Regex.Replace(file, "(PickUpBraggi\\s*=\\s*)(\\d+|-\\d+)", string.Format("{0,-25}= {1}", "PickUpBraggi", PickUpBraggi), RegexOptions.Multiline);
            }
            else
            {
                file = string.Format("{1}{0}{2,-25}= {3}", Environment.NewLine, file, "PickUpBraggi", PickUpBraggi);
            }
            if (Regex.IsMatch(file, "PickUpDancer\\s*=\\s*(\\d+|-\\d+)", RegexOptions.Multiline))
            {
                file = Regex.Replace(file, "(PickUpDancer\\s*=\\s*)(\\d+|-\\d+)", string.Format("{0,-25}= {1}", "PickUpDancer", PickUpDancer), RegexOptions.Multiline);
            }
            else
            {
                file = string.Format("{1}{0}{2,-25}= {3}", Environment.NewLine, file, "PickUpDancer", PickUpDancer);
            }
        ...
        static string SaveAutoSkillOptions(string file)
        {
            ...
            if (Regex.IsMatch(file, "UseSkillOnFollow\\s*=\\s*(\\d+|-\\d+)", RegexOptions.Multiline))
            {
                file = Regex.Replace(file, "(UseSkillOnFollow\\s*=\\s*)(\\d+|-\\d+)", string.Format("{0,-25}= {1}", "UseSkillOnFollow", UseSkillOnFollow), RegexOptions.Multiline);
            }
            else
            {
                file = string.Format("{1}{0}{2,-25}= {3}", Environment.NewLine, file, "UseSkillOnFollow", UseSkillOnFollow);
            }
            // GUI no longer writes EiraSilentBreezeLevel (handled by AI).
            // GUI no longer writes BayeriStahlHornLevel (handled by AI).
            // GUI no longer writes BayeriHailegeStarLevel (handled by AI).
            // GUI no longer writes SeraParalyzeLevel (handled by AI).
            // GUI no longer writes SeraPoisonMistLevel (handled by AI).
            
            // GUI no longer writes EleanorSonicClawLevel (handled by AI).
            // GUI no longer writes EleanorSilverveinLevel (handled by AI).
            // GUI no longer writes EleanorMidnightLevel (handled by AI).
            // GUI no longer writes DieterLavaSlideLevel (handled by AI).
        ...
        static string SaveAutobuffOptions(string file)
        {
            ...
            // GUI no longer writes BayeriSteinWandLevel (handled by AI).
        ...
        // Dual Buff System: Bard/Dancer Buff Priority
        static int _PickUpBraggi = 1;
        public static int PickUpBraggi
        {
            get { return _PickUpBraggi; }
            set { _PickUpBraggi = value; }
        }
        
        static int _PickUpDancer = 1;
        public static int PickUpDancer
        {
            get { return _PickUpDancer; }
            set { _PickUpDancer = value; }
        }
        ...
        static int _UseSkillOnFollow = 0;
        public static int UseSkillOnFollow // Set to 1 to use skills while following (not attacking)
        {
            get { return _UseSkillOnFollow; }
            set { _UseSkillOnFollow = value; }
        }
