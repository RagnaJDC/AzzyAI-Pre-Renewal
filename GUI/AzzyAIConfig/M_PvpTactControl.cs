...
//
// Description:
// This file contains partial code for the M_PvpTactControl class, which is
...
{
    public partial class M_PvpTactControl : UserControl
    {
        // The default configuration file
        string _file = "M_PVP_Tact.lua";

        // An event handler for when the tactics are changed
        public event EventHandler M_PvpTacticsChanged;

        public M_PvpTactControl()
        {
            // Check if the file does not exist
            if (File.Exists(_file))
            {
                // Load the configurations
                M_PVP_Tact.Load(_file);
            }
            else
            {
        ...
        public M_PvpTactControl(string file)
        {
            // Check if the file does not exist
            if (!File.Exists(file))
            {
        ...
        public void Open(string file)
        {
            ...
            M_PVP_Tact.Load(file);

            // Create a new tactics array
            M_PvpTact[] items = new M_PvpTact[M_PVP_Tact.Tactics.Count];

            // Copy the old tactics to the new array
            M_PVP_Tact.Tactics.CopyTo(items, 0);
        ...
        private void M_PvpTactControl_Load(object sender, EventArgs e)
        {
            // Create a new tactics array
            M_PvpTact[] items = new M_PvpTact[M_PVP_Tact.Tactics.Count];
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            ...
            if (M_PvpTacticsChanged != null)
            {
                // Raise the PvpTacticsChanged event
                M_PvpTacticsChanged(this, new EventArgs());
            }
        }
        ...
        private void buttonRemove_Click(object sender, EventArgs e)
        {
            ...
            if (listBoxTactics.SelectedItem != null)
            {
                ...
                M_PvpTact t = (M_PvpTact)listBoxTactics.SelectedItem;

                // Remove the tactic from PVP_Tact and the listBoxTactics items list
                M_PVP_Tact.Tactics.Remove(t);
                ...
                // Check if the M_PvpTacticsChanged event handler is not null
                if (M_PvpTacticsChanged != null)
                {
                    // Raise the M_PvpTacticsChanged event
                    M_PvpTacticsChanged(this, new EventArgs());
                }
            }
        }
